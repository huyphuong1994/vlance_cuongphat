<template>
  <section class="sg-blog-section item-space-rmv mt-5" v-if="lengthCounter(countBlog) > 0">
    <div class="container" :class="{ 'slider-arrows' : addons.includes('ishopet') }">
      <div
          :style="{'border-bottom': `2px solid ${settings.menu_background_color}`}"
          ...
          <div class="post slider_div">
            <div class="entry-header">
              ...
              <div class="entry-content">
                <h2 class="entry-title text-ellipse text-ellipse-two">
                  <router-link :to="{ name: 'blog.details', params: { blogId: blog.id, slug: blog.slug } }">
                    {{ blog.title }}
                  </router-link>
                </h2>
                <p class="text-ellipse text-ellipse-two">{{ blog.short_description }}</p>
                <a class="read-more" :href="getUrl('blog/' + blog.slug)"
                   @click.prevent="routerNavigator('blog.details', blog.slug)">
                  {{ lang.read_more }}
                </a>
              </div>
            </div>

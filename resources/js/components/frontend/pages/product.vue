				...
				<div class="h-100 w-100 overflow-hidden">
					<VueSlickCarousel v-bind="slick_settings">
						<template v-for="(banner, index) in banners">
							<div class="thumb" v-if="banner" :key="index">
								<a v-if="urlCheck(banner.url)" :href="banner.url" class="add-banner">
									<img :src="banner.thumbnail" class="img-fluid" style="object-fit: cover;" />
								</a>
								<router-link v-else :to="banner.url" class="add-banner">
									<img :src="banner.thumbnail" class="img-fluid" style="object-fit: cover;" />
								</router-link>
							</div>
						</template>
			...
			<li v-for="(product, index) in products" :key="index">
				...
				>
					...
					<div class="product-info">
						<h1 class="product-name text-ellipse text-ellipse-two" :title="product.product_name">
							<a :href="getUrl('product/' + product.slug)" @click.prevent="routerNavigator('product.details', product.slug)">
...
export default {
	name: "product",
	props: ["products", "grid_class", "type", "number", "banners"],
	components: {
		VueSlickCarousel,
		FlipCountdown,
		StarRating,

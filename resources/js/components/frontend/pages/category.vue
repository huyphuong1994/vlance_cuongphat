        ...
        <section class="sg-category">
            <div class="container">
                <div class="title justify-content-between">
                    <h1 v-if="is_shimmer">{{ lengthCounter(categories) > 0 ? lang.all_categories : lang.no_category_founds }}
                    </h1>
                        ...
                        <div class="category-style-2">
                            ...
                            <div class="category-list">
                                <ul class="global-list"
                                    v-if="category.child_categories && category.child_categories.length > 0">
                                    <li v-for="(child_category, child_index) in category.child_categories" :key="child_index"
                                        :class="{ 'has-baby': child_category.categories.length > 0 }">
                                        <a
                                        ...
                                        <ul v-if="child_category.categories.length > 0">
                                            <li v-for="(category, index) in child_category.categories" :key="'child_category' + index">
                                                <a
                                ...
                                <div class="category-thumb">
                                    <a :href="getUrl('category/' + category.slug)"
                                        @click.prevent="routerNavigator('product.by.category', category.slug)">
                                        <img loading="lazy" :src="category.all_popular_image" :alt="category.title"
                                            class="img-fluid">
                                    </a>
                                </div>
                            </div>

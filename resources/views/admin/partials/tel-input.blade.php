...
<div class="yoori__signup--form">
    <div class="country__code--config">
        @if ($default_country)
            <div class="country__code--config-details">
                <span class="country__code--flag">
                    <img src="{{ $default_country->flag_icon }}" alt="Flag" class="img-fluid">
                </span>
        ...
        @else
            <div class="country__code--config-details">
                <span class="country__code--flag">
                    <img src="{{ static_asset('images/flags/vn.png') }}" alt="Flag" class="img-fluid">
                </span>
                <span class="country__code--number">
                    +84
                </span>
    ...
    <ul class="country__code--list d-none">
        ...
        @foreach ($countries as $country)
            <li class="country_li" data-id="{{ $country->id }}" data-flag="{{ $country->flag_icon }}"
                ...
                <span class="country__code--flag">
                    @if ($country->flag)
                        <img src="{{ $country->flag_icon }}" alt="" class="img-fluid">
                    @else
                        <img src="{{ static_asset('images/default/default-image-40x40.png') }}" alt="default_image"
                            width="16" height="11" class="img-fluid">
                    @endif
                </span>
                <span class="country__name">
                    <strong><span class="country_name_span">{{ $country->name }}</span> <span
                            class="country__code--number">{{ str_contains($country->phonecode, '+') ? $country->phonecode : '+' . $country->phonecode }}</span></strong></span>

...
class CategoryRepository implements CategoryInterface
{
    ...
    public function all()
    {
        return Category::leftJoin('category_languages', 'category_languages.category_id', '=', 'categories.id')
            ->select(
                'categories.*',
                'category_languages.id as category_lang_id',
                'category_languages.title',
                'category_languages.lang',
                'category_languages.meta_title',
                'category_languages.meta_description'
            );
    }
    ...
    public function categoryPage()
    {
        return Category::with(['categories' => function ($query) {
            ...
            ->select('id', 'title', 'parent_id', 'logo', 'slug', 'banner')
            ->latest()->get();
    }
    ...
    public function categories($limit)
    {
        return Category::where('status', 1)
            ->select('id', 'parent_id', 'icon', 'logo', 'slug', 'banner')
            ->latest()
            ->paginate($limit);
    }
}

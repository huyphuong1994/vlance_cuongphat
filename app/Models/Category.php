...
class Category extends Model
{
    ...
    {
        return $this->getTranslation('title', languageCheck());
    }
    ...
    {
        return @is_file_exists($this->logo['image_130x95'], @$this->logo['storage'])
            ? get_media(@$this->logo['image_130x95'], @$this->logo['storage'])
            : static_asset('images/default/130x95_no_bg.png');
    }
    ...
    {
        return @is_file_exists($this->logo['image_80x60'], @$this->logo['storage'])
            ? get_media(@$this->logo['image_80x60'], @$this->logo['storage'])
            : static_asset('images/default/80x60_no_bg.png');
    }
    public function getPopularBannerAttribute()
    {
        return @is_file_exists($this->banner['image_835x200'], @$this->banner['storage'])
            ? get_media(@$this->banner['image_835x200'], @$this->banner['storage'])
            : static_asset('images/default/130x95_no_bg.png');
    }

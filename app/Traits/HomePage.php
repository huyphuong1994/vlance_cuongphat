                ...
                if (($key > $skip || $page == 1) && $key <= $content_count) {
                    foreach ($setting as $set_key => $item) {
                        if ($set_key == 'banner') {
                            ...
                            switch ($total):
                                ...
                                case (2):
                                    ...
                                    $image_1 = $media->get($item['thumbnail'][1]);
                                    $thumb = [
                                        @is_file_exists($image_0->image_variants['original_image'], $image_0->image_variants['storage']) ? @get_media($image_0->image_variants['original_image'], $image_0->image_variants['storage']) . '_0' : static_asset('images/default/default-image-620x320.png') . '_0' => $item['url'][0],
                                        @is_file_exists($image_1->image_variants['original_image'], $image_1->image_variants['storage']) ? @get_media($image_1->image_variants['original_image'], $image_1->image_variants['storage']) . '_1' : static_asset('images/default/default-image-620x320.png') . '_1' => $item['url'][1],
                                    ];
                                    array_push($banners, $thumb);
                                    break;
                                case (1):
                                    $image_0 = $media->get($item['thumbnail'][0]);
                                    $thumb = [
                                        @is_file_exists($image_0->image_variants['original_image'], $image_0->image_variants['storage']) ? @get_media($image_0->image_variants['original_image'], $image_0->image_variants['storage']) . '_0' : static_asset('images/default/default-image-1280x420.png') . '_0' => $item['url'][0],
                                    ];
                                    array_push($banners, $thumb);
                                    break;
                            endswitch;
                            $results = $this->keyDefine('banner', $key, $banners, $results);
                            $keys[] = 'banner';
                        }
                        if ($set_key == 'campaign') {
                            $component_names[] = $set_key;
                        ...
                        if ($set_key == 'category_section') {
                            ...
                            $category_products['categories'] = TopCategoryResource::collection($category->categoryByIds($item['category'], ''));
                            ...
                            $banners = [];
                            foreach ($item['banner']['thumbnail'] as $banner_index => $thumbnail) {
                                if (isset($thumbnail)) {
                                    $image = $media->get($thumbnail);
                                    $banners[] = array(
                                        'thumbnail' => $image && is_file_exists(@$image->image_variants['original_image'], @$image->image_variants['storage'])
                                            ? @get_media(@$image->image_variants['original_image'], $image->image_variants['storage'])
                                            : static_asset('images/default/default-image-405x745.png'),
                                        'url' => $item['banner']['url'][$banner_index]
                                    );
                                }
                            }
                            $category_products['setting'] = array(
                                'bg'                    => $item['bg'] ?? 'white',
                                'type'                  => intval($item['type']) ?? 2,
                                'title_active_color'    => $item['title_active_color'] ?? 'black',
                                'title_color'           => $item['title_color'] ?? 'black',
                            );
                            $category_products['banners'] = $banners;
                            $results = $this->keyDefine('category_section', $key, $category_products, $results);
                            $keys[] = 'category_section';
                        }
